...
import com.klifora.shop.utils.getErrorMessage
import com.klifora.shop.utils.hideKeyboard
import com.klifora.shop.utils.ioDispatcher
import com.klifora.shop.utils.mainThread
import com.klifora.shop.utils.parseResult
...
class Repository @Inject constructor(
    ...
    suspend fun <T> callApiTranslate(
        ...
        CoroutineScope(Dispatchers.IO + coRoutineExceptionHandler + Job()).launch {
            flow {
                emit(callHandler.sendRequestTranslate(apiInterface = apiTranslateInterface) as Response<*>)
            }.flowOn(ioDispatcher)
                .retryWhen { cause, attempt ->
                    (attempt < RETRY_COUNT) && (cause is IOException)
                }.onStart {
